...
| **Tải đủ hàng trăm nghìn dòng nhưng Cloudflare chỉ có 1 list nhỏ (VD: 300 entries), log có dòng "Maximum number of blocked domains reached" ngay sau khi mới đọc file** | **Secret `CLOUDFLARE_LIST_ITEM_LIMIT` đang bị đặt nhầm thành số quá nhỏ** (VD: `300` thay vì để trống hoặc `300000`) - không phải bug code | Kiểm tra Settings → Secrets and variables → Actions → Secrets → `CLOUDFLARE_LIST_ITEM_LIMIT`. Xoá secret này (dùng mặc định 300000) hoặc sửa lại đúng số. Từ bản mới nhất, script sẽ **tự in cảnh báo to** ngay khi phát hiện giá trị này quá nhỏ, không cần đoán mò nữa |
| **Job "Incrementally sync rules and lists" crash với `Error: undefined`, retry 50 lần rất lâu rồi sập bằng stack trace khó hiểu** | Bug thật đã sửa: hàm xử lý lỗi API luôn hiện `undefined` thay vì lý do thật (thường là *"đã đạt giới hạn 300 list/tài khoản"*), và lỗi 400 (cố định, không tự hết) vẫn bị retry như lỗi tạm thời | Đã sửa 3 điểm: (1) hiện đúng lý do lỗi thật từ Cloudflare, (2) dừng ngay khi gặp 400 từ Cloudflare thay vì retry 50 lần vô ích, (3) không còn crash bằng unhandled exception - báo lỗi rõ ràng + thoát có kiểm soát (vẫn báo đỏ nếu thật sự có domain không đồng bộ được, không giả vờ thành công) |
| **Muốn tự điều chỉnh giới hạn số domain/IP tối đa theo gói Cloudflare (Free/Pro/Enterprise)** | Đã có sẵn, không cần thêm biến mới | Dùng chính secret `CLOUDFLARE_LIST_ITEM_LIMIT` (mặc định 300000 cho Free/Pro) - đặt giá trị khác nếu bạn dùng gói Enterprise với hạn mức cao hơn |
