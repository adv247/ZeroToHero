...
import { CLOUDFLARE_RATE_LIMIT_BACKOFF_SCHEDULE, LIST_COUNT_LIMIT, RATE_LIMITING_HTTP_ERROR_CODE } from "./constants.js";
...
export const downloadFiles = async (filePath, urls) => {
  ...
  for (const url of urls) {
    try {
      ...
      //
      // BUG THẬT ĐÃ SỬA (gây lỗi "Detected unsettled top-level await" ở bản
      // trước): KHÔNG được vừa gắn `.on("data")` để tự tay ghi vào cache VỪA
      // dùng `pipeline()` để ghi vào file merge trên CÙNG 1 stream - 2 cơ chế
      // tiêu thụ dữ liệu cùng lúc trên 1 Readable stream sẽ tranh chấp nhau,
      // khiến `pipeline()` không bao giờ resolve đúng (Promise bị "treo" vĩnh
      // viễn -> "unsettled top-level await" -> job thất bại y hệt log thật).
      // Cách sửa AN TOÀN: tải về 1 file tạm DUY NHẤT bằng pipeline() sạch sẽ
      // (chỉ 1 consumer), sau khi tải xong HẲN mới đọc lại file tạm đó (đồng
      // bộ, không phải stream) để copy sang CẢ file merge lẫn file cache -
      // không bao giờ có 2 consumer tranh nhau trên cùng 1 stream nữa.
      const nodeStream = Readable.fromWeb(response.body);
      const tempFilePath = `${cachedFilePath}.tmp`;
      const tempWriteStream = createWriteStream(tempFilePath, { flags: "w" });

      // QUAN TRỌNG: KHÔNG cần (và KHÔNG được) `await once(tempWriteStream,
      // "finish")` sau dòng này - `pipeline()` khi KHÔNG truyền {end: false}
      // đã TỰ chờ writeStream ghi xong hẳn (bao gồm cả sự kiện "finish")
      // trước khi resolve. Gọi `once(..., "finish")` sau đó là chờ 1 sự kiện
      // ĐÃ XẢY RA TRONG QUÁ KHỨ - EventEmitter không bao giờ bắn lại, nên
      // Promise treo vĩnh viễn -> "Detected unsettled top-level await" (bug
      // thật gây lỗi ở bản v17, tái hiện và xác nhận bằng test thực tế).
      await pipeline(nodeStream, tempWriteStream);

      const downloaded = readFileSync(tempFilePath);
      const bytes = downloaded.length;

      // Dùng fs ĐỒNG BỘ (không phải stream) để copy sang 2 đích - đơn giản,
      // an toàn, không có rủi ro race condition/dual-consumer nào cả vì
      // đây không phải thao tác trên stream đang chảy nữa.
      appendFileSync(filePath, downloaded);
      appendFileSync(filePath, "\n");
      writeFileSync(cachedFilePath, downloaded);
...
export async function notifySyncReport(stats) {
  ...
  const capacityWarning = typeof totalAccountListsCount === "number" && totalAccountListsCount > LIST_COUNT_LIMIT - 10;

  // BÁO CÁO CẮT BỚT AN TOÀN: khi đã chạm giới hạn 300 list/tài khoản, sync
  // vẫn HOÀN TẤT THÀNH CÔNG (không crash) nhưng một phần dữ liệu bị cắt bớt
  // có chủ đích - phải báo cáo RÕ RÀNG số lượng thật, không được im lặng.
  const truncationLine = truncatedCount > 0
    ? `\n⚠️ ${truncatedCount.toLocaleString()} mục KHÔNG được thêm vì đã chạm giới hạn 300 list/tài khoản (bị cắt bớt an toàn, không phải lỗi) - chạy defragment-lists.yml để giải phóng chỗ trống.\n`
    : "";
  ...
  const capacityLine = capacityWarning
    ? `\n💥 Dung lượng đã vượt ngưỡng an toàn (${totalAccountListsCount}/${LIST_COUNT_LIMIT} lists toàn tài khoản)! Hãy bớt bớt nguồn blocklist hoặc chạy defragment ngay.\n`
    : "";
  ...
  const discordMessage =
    ...
    `• Tổng số list: \`${currentListsCount}\`` + (typeof totalAccountListsCount === "number" ? ` (toàn tài khoản: \`${totalAccountListsCount}/${LIST_COUNT_LIMIT}\`)` : "") + `\n` +
    "```diff\n" +
  ...
  const telegramMessage =
    ...
    (capacityWarning ? `\n⚠️ CẢNH BÁO: dung lượng đã vượt ngưỡng an toàn (${totalAccountListsCount}/${LIST_COUNT_LIMIT})! Hãy bớt bớt nguồn blocklist hoặc chạy defragment ngay.\n` : "") +
    (truncatedCount > 0 ? `\n⚠️ ${truncatedCount.toLocaleString()} mục KHÔNG được thêm (đã chạm giới hạn 300 list, bị cắt bớt an toàn) - chạy defragment-lists.yml để giải phóng chỗ trống.\n` : "") +
    `\n⚡ <b>Hiệu năng</b>\n` +
    `- Chế độ: ${fastMode ? "FAST_MODE (song song)" : "Tuần tự"}\n` +
    `- Thời gian xử lý: ${executionTime}\n` +
    `- Số lần tự lùi lại (retry): ${retryCount}\n` +
